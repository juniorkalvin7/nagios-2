...
if len(args)<2:
  ...
  community=args[1]
try:
  in_bits=(commands.getoutput("/usr/bin/snmpwalk -t 100000 -v 1 -c %s %s:3401 1.3.6.1.4.1.3495.1.3.2.1.4" %(community,str(host)))).split(': ')[1]
  out_bits=(commands.getoutput("/usr/bin/snmpwalk -t 100000 -v 1 -c %s %s:3401 1.3.6.1.4.1.3495.1.3.2.1.5" %(community,str(host)))).split(': ')[1]
except Exception,e:
...
arquivo="/var/lib/centreon/centplugins/squid_%s_http_in_ou_bytes" % (str(host))
...
print "Squid - HTTP In/Out Bytes Traffic In : %s %sb/s (%s %%), Out : %s %sb/s (%s %%) -" %\
