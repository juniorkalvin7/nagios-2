...
if len(args) == 3:
		...
		con = commands.getoutput('snmpwalk -v 2c -c %s %s %s'%(community,ip, memoria_total))
		if "No Response" in str(con):
                        print "CRITICAL - Sem comunicação via snmp: %s"%str(con)
                        sys.exit(2)
		...
		estado_vms = ""
				...
				estado_vms += "%s-[%s], "%(nome, estado) 

                string = string + '</table>'
                string = string.replace('\n','')

                print 'OK - Estado das VMs: %s \n %s '%(estado_vms[:len(estado_vms)-2],string)
                saida = 0
                ...
                if "No Response" in str(con):
                        ...
                        if (len(lista_nome_disco) == len(lista_tam_total) == len(lista_tam_usado)):
                                for i in range(0, len(lista_nome_disco)):
                                        ...
                                        if float(porcentagem) >= 80.0 and porcentagem < 90.0 and saida !=2:
                                                #print porcentagem
                                                saida = 1
                                        elif float(porcentagem) >= 90.0:
                                                saida = 2
                                       ...
                                       # graf = graf + " hrStorageIndex-FixedDisk.%s=%.2f;; "%(nome,porcentagem)
                                        graf = graf + " hrStorageIndex-FixedDisk.%s=%.2f;; "%(nome,float(tam_usado)/1024)
                        if saida == 1:
                                msg = "WARNING - Disco(s) com mais de 80% de ocupa&#231&#227o | "+graf
                        if saida == 2:
                                msg = "CRITICAL - Disco(s) com mais de 90% de ocupa&#231&#227o | "+graf
                        if saida == 0:
