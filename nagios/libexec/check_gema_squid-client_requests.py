...
if len(args)<2:
  ...
  community=args[1]
try:
  status=(commands.getoutput("/usr/bin/snmpwalk -t 100000 -v 1 -c %s %s:3401 .1.3.6.1.4.1.3495.1.3.2.1.1" %(community,str(host)))).split('Counter32: ')[1]
except Exception,e:
...
print "Squid - Client Requests - %d|squidstat=%d squidstatb=%d" % (valor,valor,valor)
