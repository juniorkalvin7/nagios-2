...
my $response;
# Path to  tmp files
my $TRAFFIC_FILE = "/var/lib/centreon/centplugins/";
...
if ( !$iface_speed ) {
	# Do nothing
        ...
        #gemayellira 
        if ($units eq 'm'){
            $iface_speedF=($iface_speed*1000000);
        }elsif($units eq 'k'){
            $iface_speedF=($iface_speed*500000);
        }elsif($units eq 'g'){
            $iface_speedF=($iface_speed*2000000);
        }elsif($units eq 'b'){
            $iface_speedF=($iface_speed*250000);
        }
	...
	if ( $iface_speedOut ) {
                if ($units eq 'm'){
                    $iface_speedOutF=($iface_speedOut*1000000);
                }elsif($units eq 'k'){
                    $iface_speedOutF=($iface_speedOut*500000);
                }elsif($units eq 'g'){
                    $iface_speedOutF=($iface_speedOut*2000000);
                }elsif($units eq 'b'){
                    $iface_speedOutF=($iface_speedOut*250000);
                }
                $iface_speedOut = bits2bytes( $iface_speedOut, $units ) / 1024;

	}else{
            $iface_speedOutF=$iface_speedF;
        }
	if ( !$max_value ) {
...
#my $cccc=0;
#$cccc  = $response->{ $snmpIfInOctets . "." . $iface_number }; # in kiloBytes
#print $cccc;
...
#print $in_bytes.':'.$last_in_bytes.':'.$max_bytes.'
#';
...
my $bytesin=sprintf( "%.0lf",(( $in_bytes - $last_in_bytes ) / ( time - $last_check_time ))*1000*10);
my $bytesout=sprintf( "%.0lf",(( $out_bytes - $last_out_bytes ) / ( time - $last_check_time ))*1000*10);
#print $bytesin;
...
$out_traffic = $out_traffic*10;
$in_traffic= $in_traffic*10;
#$in_usage = $in_usage * 1024;
#print $out_traffic.' '.$out_bytes.' '.$out_traffic_absolut.'  ';
...
"|inUsage=$in_usage%;$warn_usage;$crit_usage outUsage=$out_usage%;$warn_usage;$crit_usage"
  . " inBandwidth=" . $bytesin .',0'.$suffix.';0;' . $iface_speedF." outBandwidth=" . $bytesout . ',0'. $suffix.';0;' . $iface_speedOutF
   . " linkDownload=".$iface_speedF. ',0'.$suffix.';0;' . $iface_speedF." linkUpload=". $iface_speedOutF . ',0'. $suffix.';0;' . $iface_speedOutF;
stop($output, $state);
...
Exemplos:
Especificacao de tamanho de link para download|upload diferentes link asincrono:
link de 1G download 100M upload
./check_iftraffic3.pl -H 127.0.0.1 -i eth0 -w 80 -c 90 -C COMMUNITY -u m -I 1024 -O 100
link de 100megas download 10M upload
./check_iftraffic3.pl -H 127.0.0.1 -i eth0 -w 80 -c 90 -C COMMUNITY -u m -I 100 -O 10
link de 1mega download 100kb upload
./check_iftraffic3.pl -H 127.0.0.1 -i eth0 -w 80 -c 90 -C COMMUNITY -u k -I 1024 -O 100
link de 600Kbps download 60Kbps upload
./check_iftraffic3.pl -H 127.0.0.1 -i eth0 -w 80 -c 90 -C COMMUNITY -u k -I 600 -O 60
Especificacao de tamanho de link para download|upload iguais link sincrono:
link de 1G
./check_iftraffic3.pl -H 127.0.0.1 -i eth0 -w 80 -c 90 -C COMMUNITY -u g -b 1
link de 100megas
./check_iftraffic3.pl -H 127.0.0.1 -i eth0 -w 80 -c 90 -C COMMUNITY -u m -b 100
link de 1mega
./check_iftraffic3.pl -H 127.0.0.1 -i eth0 -w 80 -c 90 -C COMMUNITY -u m -b 1
link de 512kb
./check_iftraffic3.pl -H 127.0.0.1 -i eth0 -w 80 -c 90 -C COMMUNITY -u k -b512
Help Padrao:
    ...
    -v, 1,2 ou 3
	Insira vercao do snmp por gemayellira 
    -L, --list FLAG (on/off)
